...
class Customlib
{
    ...
    public function getBaseUrl()
    {
        ...
        if ($admin && !empty($admin['db_array']['base_url'])) {
            $base_url = $admin['db_array']['base_url'];
        } else if ($student && !empty($student['db_array']['base_url'])) {
            $base_url = $student['db_array']['base_url'];
        } else {
            $base_url = "";
        }

        // If DB has localhost but site is hosted on live domain, use dynamic base_url
        if (strpos($base_url, 'localhost') !== false && isset($_SERVER['HTTP_HOST']) && strpos($_SERVER['HTTP_HOST'], 'localhost') === false) {
            $base_url = base_url();
        }
    ...
    public function getFolderPath()
    {
        ...
        if ($admin && !empty($admin['db_array']['folder_path'])) {
            $folder_path = $admin['db_array']['folder_path'];
        } else if ($student && !empty($student['db_array']['folder_path'])) {
            $folder_path = $student['db_array']['folder_path'];
        } else {
            $folder_path = null;
        }

        // If the path from DB does not exist on this server (e.g. C:\xampp\... migrated to Linux server), fallback to null (relative root)
