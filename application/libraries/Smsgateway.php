...
class Smsgateway
{
    ...
    public function getGroupAddFeeContent($data, $template, $sms_detail_type = null)
    {
     $currency_symbol      = $this->sch_setting[0]['currency_symbol'];
        ...
        $clean_payment_id     = implode(', ', $payment_id);
        $data['payment_id']   = $clean_payment_id;
        $data['receipt_no']   = $clean_payment_id;
        $data['invoice_no']   = $clean_payment_id;
        $data['invoice_id']   = $clean_payment_id;
        $data['class']        = $fee->class;
        $data['section']      = $fee->section;
        $total_fee_str        = $currency_symbol . amountFormat($fee_amount);
        $data['fee_amount']   = $total_fee_str;
        $data['amount']       = $total_fee_str;
        $data['paid_amount']  = $total_fee_str;
        $data['total_amount'] = $total_fee_str;
        if (isset($data['type'])) {
            $data['type']     = trim(str_replace(',', ', ', $data['type']), '()');
            $data['fee_type'] = $data['type'];
        }
        if (isset($data['fee_group_name'])) {
            $data['fee_group_name'] = trim(str_replace(',', ', ', $data['fee_group_name']), '()');
        }
        $data['student_name'] = $this->_CI->customlib->getFullName($fee->firstname, $fee->middlename, $fee->lastname, $this->sch_setting[0]['middlename'], $this->sch_setting[0]['lastname']); 
