                            ...
                            <table class="table table-hover table-striped table-bordered loading1" id="exam_table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="white-space-nowrap"><?php echo $this->lang->line('session'); ?></th>
                                        <th class="text-center white-space-nowrap"><?php echo $this->lang->line('subjects_included'); ?></th>
                                        <th class="text-center white-space-nowrap"><?php echo $this->lang->line('publish_exam'); ?></th>
                                        <th class="text-center white-space-nowrap"><?php echo $this->lang->line('publish_result'); ?></th>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.examTeacherReamark', function () {
    ...
    $(document).on('change', '#bulk_class_id', function (e) {
        var class_id = $(this).val();
        var selector = $(this).closest("form").find('#bulk_section_id');
        getSectionByClass(class_id, 0, selector);
    });

    $(document).on('submit', '#bulkUploadMarksForm', function (e) {
        e.preventDefault();
        var $btn = $('#btnBulkUpload');
        var form_data = new FormData(this);

        $.ajax({
            url: baseurl + "admin/examgroup/uploadbulkfile",
            type: 'POST',
            dataType: 'JSON',
            data: form_data,
            contentType: false,
    ...
    $(document).on('click', '#btnExportBulkSample', function (e) {
        e.preventDefault();
        var form = $(this).closest('form');
        var exam_id = form.find('input[name="exam_id"]').val();
        var class_id = form.find('#bulk_class_id').val();
        var section_id = form.find('#bulk_section_id').val();
        var session_id = form.find('#bulk_session_id').val();

        if (!class_id || !section_id) {
            errorMsg("Please select Class and Section first to download sample CSV.");
            return;
        }

        var url = baseurl + "admin/examgroup/exportbulkformat?exam_id=" + exam_id + "&class_id=" + class_id + "&section_id=" + section_id + "&session_id=" + session_id;
