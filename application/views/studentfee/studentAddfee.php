...
?>
                        ...
                        <div class="row no-print mb10">
                            <div class="col-md-12 mDMb10">
                                ...
                                <button type="button" class="btn btn-sm btn-success schoolerPrintSelected" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Please wait"><i class="fa fa-print"></i> Schooler Print</button>
...
foreach ($student_due_fee as $key => $fee) {
    $fee_group_key = "fee_group_" . $key;
    ?>
                                    <tr class="info">
                                        <td>
                                            <input class="fee-group-select-all" type="checkbox" data-fee-group-key="<?php echo $fee_group_key; ?>">
                                        </td>
                                        <td colspan="13" align="left" class="text-rtl-right">
                                            <strong><?php echo $fee->name; ?></strong> - <?php echo $this->lang->line('select_all'); ?>
                                        </td>
                                    </tr>
                                    <?php
...
if ($feetype_balance > 0 && strtotime($fee_value->due_date) < strtotime(date('Y-m-d'))) {
        ...
        ?>
                                                <td>
          <input class="checkbox fee-group-fee-checkbox" type="checkbox" name="fee_checkbox" data-fee-group-key="<?php echo $fee_group_key; ?>" data-fee_master_id="<?php echo $fee_value->id ?>" data-fee_session_group_id="<?php echo $fee_value->fee_session_group_id ?>" data-fee_groups_feetype_id="<?php echo $fee_value->fee_groups_feetype_id ?>" data-fee_category="fees"  data-trans_fee_id="0">
                                                </td>
...
<div id="schoolerPrintPasswordModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><i class="fa fa-lock"></i> Schooler Print Authentication</h4>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <label for="schooler_print_password">Enter Password to proceed:</label>
                    <input type="password" class="form-control" id="schooler_print_password" placeholder="Password" autocomplete="off">
                    <span id="schooler_password_error" class="text-danger" style="display:none;"></span>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-success" id="schooler_print_submit"><i class="fa fa-print"></i> Print</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function Popup(data, winload = false)
    {
        ...
        var frameWin = frame.contentWindow || frame;
        var doc = frameWin.document || frame.contentDocument;

        doc.open();

        var content = '';
        if (data.indexOf('<!DOCTYPE') === -1 && data.indexOf('<!doctype') === -1) {
        ...
        if (data.indexOf('<html') > -1) {
            ...
            content += '<html lang="en"><head><title></title>';
            content += '<link rel="stylesheet" href="' + base_url + 'backend/bootstrap/css/bootstrap.min.css">';
            content += '<link rel="stylesheet" href="' + base_url + 'backend/dist/css/font-awesome.min.css">';
            content += '<link rel="stylesheet" href="' + base_url + 'backend/dist/css/ionicons.min.css">';
            content += '<link rel="stylesheet" href="' + base_url + 'backend/dist/css/AdminLTE.min.css">';
            content += '<link rel="stylesheet" href="' + base_url + 'backend/dist/css/print_fee_receipt.css">';
            content += '</head><body>';
            content += data;
            content += '</body></html>';
        }

        if (content.indexOf('print_fee_receipt.css') === -1) {
            content = content.replace('<head>', '<head><link rel="stylesheet" href="' + base_url + 'backend/dist/css/print_fee_receipt.css">');
        }
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.collectSelected', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var date_format = '<?php echo strtr($this->customlib->getSchoolDateFormat(), array("d" => "dd", "m" => "mm", "Y" => "yyyy")); ?>';
                    var $collectDate = $("#listCollectionModal").find("input.date_fee");
                    $collectDate.removeAttr("readonly");
                    if ($collectDate.data('datepicker')) {
                        $collectDate.datepicker('destroy');
                    }
        ...
        $(document).on('click', '.schoolerPrintSelected', function () {
            var $this = $(this);
            var array_to_print = [];
            $.each($("input[name='fee_checkbox']:checked"), function () {
                var trans_fee_id = $(this).data('trans_fee_id');
            ...
            if (array_to_print.length === 0) {
                ...
                $('#schooler_print_password').val('');
                $('#schooler_password_error').hide().text('');
                $('#schoolerPrintPasswordModal').data('fees_data', JSON.stringify(array_to_print));
                $('#schoolerPrintPasswordModal').modal('show');
            }
        });

        $(document).on('click', '#schooler_print_submit', function () {
            var $this = $(this);
            var entered_password = $('#schooler_print_password').val();
            var schooler_password = 'SVN@1998C#';
            if (entered_password !== schooler_password) {
                $('#schooler_password_error').text('Incorrect Password').show();
                return;
            }
            $('#schooler_password_error').hide();
            var fees_data = $('#schoolerPrintPasswordModal').data('fees_data');
            $.ajax({
                url: '<?php echo site_url("studentfee/schoolerPrintByGroupArray") ?>',
                type: 'post',
                data: {'data': fees_data},
                beforeSend: function () {
                    $this.button('loading');
                ...
                success: function (response) {
                    $('#schoolerPrintPasswordModal').modal('hide');
                    Popup(response);
                    $this.button('reset');
                },
                error: function (xhr) {
        ...
        $('#schoolerPrintPasswordModal').on('keypress', function (e) {
            if (e.which === 13) {
                e.preventDefault();
                $('#schooler_print_submit').click();
            }
        });
    });
...
$(document).on('change', '.fee-group-select-all', function () {
    var groupKey = $(this).data('fee-group-key');
    $('input[name="fee_checkbox"][data-fee-group-key="' + groupKey + '"]').prop('checked', this.checked);
});

$(document).on('change', 'input[name="fee_checkbox"][data-fee-group-key]', function () {
    var groupKey = $(this).data('fee-group-key');
    var $groupCheckboxes = $('input[name="fee_checkbox"][data-fee-group-key="' + groupKey + '"]');
    var checkedCount = $groupCheckboxes.filter(':checked').length;
    $('.fee-group-select-all[data-fee-group-key="' + groupKey + '"]').prop('checked', checkedCount === $groupCheckboxes.length && $groupCheckboxes.length > 0);
});
</script>
