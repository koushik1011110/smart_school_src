...
class Staffattendancemodel extends MY_Model {
    ...
    public function searchStaffattendance($date, $staff_id, $active_staff = true) {
        ...
        if ($q->num_rows() == 0) {
            ...
            $attendance = $q->row();
            if (!empty($data['created_at']) && empty($attendance->check_out_time)) {
                $this->db->where('id', $attendance->id);
                $this->db->update('staff_attendance', array(
                    'check_out_time' => date('H:i:s', strtotime($data['created_at'])),
