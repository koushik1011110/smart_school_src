...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function fee_deposit_bulk($bulk_data, $student_fees_discount_id = null)
    {
        ...
        if ($this->db->trans_status() === false) {
            ...
            // Custom SMS API Trigger for Bulk Deposit
            try {
                if (!empty($bulk_data) && is_array($fees_return)) {
                    foreach ($fees_return as $index => $ret_item) {
                        $raw_item = $bulk_data[$index] ?? null;
                        if ($raw_item) {
                            $amt_info = $raw_item['amount_detail'] ?? array();
                            $paid_amt = is_array($amt_info) ? ($amt_info['amount'] ?? 0) : 0;
                            $p_date = is_array($amt_info) ? ($amt_info['date'] ?? date('Y-m-d')) : date('Y-m-d');
                            $sfm_id = $raw_item['student_fees_master_id'] ?? null;
                            $fgft_id = $raw_item['fee_groups_feetype_id'] ?? null;
                            $stf_id = $raw_item['student_transport_fee_id'] ?? null;
                            $this->send_fee_sms_custom($sfm_id, $fgft_id, $stf_id, $paid_amt, $ret_item['invoice_id'], $ret_item['sub_invoice_id'], $p_date);
                        }
                    }
                }
            } catch (Exception $e) {
                // Silently fail to not disrupt fee deposit flow
            }
    ...
    public function fee_deposit($data, $send_to, $student_fees_discount_id = null)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($this->db->trans_status() === false) {
                ...
                // Custom SMS API Trigger
                try {
                    $amt_info = is_array($data['amount_detail']) ? $data['amount_detail'] : json_decode($data['amount_detail'], true);
                    $curr_dep = is_array($amt_info) ? end($amt_info) : array();
                    $paid_amt = $curr_dep['amount'] ?? 0;
                    $p_date = $curr_dep['date'] ?? date('Y-m-d');
                    $sfm_id = $data['student_fees_master_id'] ?? null;
                    $fgft_id = $data['fee_groups_feetype_id'] ?? null;
                    $stf_id = $data['student_transport_fee_id'] ?? null;
                    $this->send_fee_sms_custom($sfm_id, $fgft_id, $stf_id, $paid_amt, $row->id, $inv_no, $p_date);
                } catch (Exception $e) {
                    // Silently fail to not disrupt fee deposit flow
                }
            ...
            if ($this->db->trans_status() === false) {
                ...
                try {
                    ...
                    $fgft_id = $data['fee_groups_feetype_id'] ?? null;
                    $stf_id = $data['student_transport_fee_id'] ?? null;
                    $this->send_fee_sms_custom($sfm_id, $fgft_id, $stf_id, $paid_amt, $inserted_id, 1, $p_date);
                } catch (Exception $e) {
                    // Silently fail to not disrupt fee deposit flow
                }
    ...
    public function send_fee_sms_custom($student_fees_master_id, $fee_groups_feetype_id, $student_transport_fee_id, $paid_amount, $invoice_id, $sub_invoice_id, $payment_date)
    {
        try {
            $student_id = 0;
            $student_name = '';
            ...
            if (!empty($student_fees_master_id) && !empty($fee_groups_feetype_id)) {
                $fee_detail = $this->getFeesByStudentFeeMasterAndFeetype($student_fees_master_id, $fee_groups_feetype_id);
                if ($fee_detail) {
                    $student_id = $fee_detail->student_id;
                    $student_name = trim(($fee_detail->firstname ?? '') . ' ' . ($fee_detail->middlename ?? '') . ' ' . ($fee_detail->lastname ?? ''));
                    $fee_type = !empty($fee_detail->type) ? $fee_detail->type : (!empty($fee_detail->code) ? $fee_detail->code : ($fee_detail->name ?? 'School Fee'));
                    $total_fee = (float) ($fee_detail->amount ?? 0);

                    if (!empty($fee_detail->amount_detail) && $fee_detail->amount_detail != '0') {
                        $amount_detail_arr = json_decode($fee_detail->amount_detail, true);
                        if (is_array($amount_detail_arr)) {
                            foreach ($amount_detail_arr as $d_item) {
                                $total_paid += (float) ($d_item['amount'] ?? 0) + (float) ($d_item['amount_discount'] ?? 0);
                            }
                        }
                    }
                }
            } elseif (!empty($student_transport_fee_id)) {
                $fee_detail = $this->getTransportFeeByID($student_transport_fee_id);
                if ($fee_detail) {
                    $student_id = $fee_detail->student_id;
                    $student_name = trim(($fee_detail->firstname ?? '') . ' ' . ($fee_detail->middlename ?? '') . ' ' . ($fee_detail->lastname ?? ''));
                    $fee_type = "Transport Fee" . (!empty($fee_detail->month) ? " (" . $fee_detail->month . ")" : "");
                    $total_fee = (float) ($fee_detail->fees ?? 0);
            ...
            if ($student_id > 0) {
                $student_rec = $this->db->select('firstname, middlename, lastname, mobileno, guardian_phone')->from('students')->where('id', $student_id)->get()->row();
                if ($student_rec) {
                    if (empty($student_name)) {
                        $student_name = trim(($student_rec->firstname ?? '') . ' ' . ($student_rec->middlename ?? '') . ' ' . ($student_rec->lastname ?? ''));
                    }
                    $mobile_no = !empty($student_rec->mobileno) ? $student_rec->mobileno : $student_rec->guardian_phone;
                }

                $student_session_rec = $this->db->select('id')->from('student_session')->where('student_id', $student_id)->where('session_id', $this->current_session)->get()->row();
                if (!$student_session_rec) {
                    $student_session_rec = $this->db->select('id')->from('student_session')->where('student_id', $student_id)->order_by('id', 'DESC')->get()->row();
                }

                if ($student_session_rec) {
                    $st_session_id = $student_session_rec->id;
                    $student_fee_list = $this->getStudentFees($st_session_id);
                    $tot_assigned = 0;
                    $tot_paid = 0;

                    if (!empty($student_fee_list)) {
                        foreach ($student_fee_list as $m_fee) {
                            if (!empty($m_fee->fees) && is_array($m_fee->fees)) {
                                foreach ($m_fee->fees as $f_item) {
                                    $f_code = isset($f_item->code) ? trim($f_item->code) : '';
                                    $f_type = isset($f_item->type) ? trim($f_item->type) : '';
                                    $f_ftid = isset($f_item->feetype_id) ? trim($f_item->feetype_id) : '';
                                    if ($f_code === '01' || $f_code === '1' || $f_type === '01' || $f_type === '1' || $f_ftid === '01' || $f_ftid === '1') {
                                        continue;
                                    }
                                    $tot_assigned += (float) ($f_item->amount ?? 0);
                                    if (!empty($f_item->amount_detail) && $f_item->amount_detail != '0') {
                                        $arr = json_decode($f_item->amount_detail, true);
                                        if (is_array($arr)) {
                                            foreach ($arr as $dep) {
                                                $tot_paid += (float) ($dep['amount'] ?? 0) + (float) ($dep['amount_discount'] ?? 0);
                                            }
                                        }
                                    }
                                }
                    ...
                    $trans_list = $this->getTransStudentFees($st_session_id);
                    if (!empty($trans_list)) {
                        foreach ($trans_list as $t_fee) {
                            $tot_assigned += (float) ($t_fee->amount ?? 0);
                            if (!empty($t_fee->amount_detail) && $t_fee->amount_detail != '0') {
                                $t_arr = json_decode($t_fee->amount_detail, true);
                                if (is_array($t_arr)) {
                                    foreach ($t_arr as $t_dep) {
                                        $tot_paid += (float) ($t_dep['amount'] ?? 0) + (float) ($t_dep['amount_discount'] ?? 0);
                                    }
                                }
                            }
                        }
                    ...
                    $overall_pending = max(0, $tot_assigned - $tot_paid);
                }
            }

            // Clean mobile number to exact 10 digits
            $clean_mobile = preg_replace('/[^0-9]/', '', $mobile_no);
            if (strlen($clean_mobile) > 10) {
                $clean_mobile = substr($clean_mobile, -10);
            }

            if (!empty($clean_mobile) && strlen($clean_mobile) == 10) {
                $pending_payment_str = (floor($overall_pending) == $overall_pending) ? number_format($overall_pending, 0, '.', '') : number_format($overall_pending, 2, '.', '');
                $amount_paid_val = (float) $paid_amount;
                $amount_paid_str = (floor($amount_paid_val) == $amount_paid_val) ? number_format($amount_paid_val, 0, '.', '') : number_format($amount_paid_val, 2, '.', '');
                $receipt_no = $invoice_id . '/' . $sub_invoice_id;

                $formatted_date = !empty($payment_date) ? date('d-m-Y', strtotime($payment_date)) : date('d-m-Y');

                $school_name = 'SVN KATHIATALI';

                $message = "Dear Parent,\nFee payment has been received successfully.\nStudent: " . $student_name . "\nFee Type: " . $fee_type . "\nAmount Paid: Rs" . $amount_paid_str . "\nPending Payment: " . $pending_payment_str . "\nReceipt No: " . $receipt_no . "\nDate: " . $formatted_date . "\nThank you.\n- " . $school_name . "\n\nPowerd by KKWEBMART\nDE";

                $api_url = "http://bulk.powerstext.in/http-tokenkeyapi.php?authentic-key=34384b4b5745424d4152543130301773909499&senderid=DSAENT&route=1&number=" . $clean_mobile . "&message=" . urlencode($message) . "&templateid=1677100000000388570";
            ...
            // Silently fail to avoid breaking fee submission flow
        }
    }

    public function send_fee_sms_custom_bulk($collected_fees, $raw_data)
    {
        try {
            if (empty($collected_fees) || !is_array($collected_fees)) {
                return;
            }
            ...
            foreach ($collected_fees as $c_idx => $c_item) {
                $raw_item = $raw_data[$c_idx] ?? null;
                $sfm_id = $c_item['fee_groups_feetype_id'] ? ($raw_item['student_fees_master_id'] ?? null) : null;
                $fgft_id = $c_item['fee_groups_feetype_id'] ?? null;
                $stf_id = $c_item['student_transport_fee_id'] ?? null;

                if ($sfm_id && $fgft_id) {
                    $fee_detail = $this->getFeesByStudentFeeMasterAndFeetype($sfm_id, $fgft_id);
                    if ($fee_detail) {
                        if (!$student_id) {
                            $student_id = $fee_detail->student_id;
                            $student_name = trim(($fee_detail->firstname ?? '') . ' ' . ($fee_detail->middlename ?? '') . ' ' . ($fee_detail->lastname ?? ''));
                        }
                        $type_name = !empty($fee_detail->type) ? $fee_detail->type : (!empty($fee_detail->code) ? $fee_detail->code : ($fee_detail->name ?? 'School Fee'));
                        if (!in_array($type_name, $fee_types)) {
                            $fee_types[] = $type_name;
                        }
                    }
                } elseif ($stf_id) {
                    $fee_detail = $this->getTransportFeeByID($stf_id);
                    if ($fee_detail) {
                        if (!$student_id) {
                            $student_id = $fee_detail->student_id;
                            $student_name = trim(($fee_detail->firstname ?? '') . ' ' . ($fee_detail->middlename ?? '') . ' ' . ($fee_detail->lastname ?? ''));
                        }
                        $type_name = "Transport Fee" . (!empty($fee_detail->month) ? " (" . $fee_detail->month . ")" : "");
                        if (!in_array($type_name, $fee_types)) {
                            $fee_types[] = $type_name;
                        }
                    }
                ...
                if ($raw_item) {
                    $amt_info = is_array($raw_item['amount_detail']) ? $raw_item['amount_detail'] : json_decode($raw_item['amount_detail'], true);
                    $curr_dep = is_array($amt_info) ? end($amt_info) : array();
                    $total_paid_amount += (float) ($curr_dep['amount'] ?? 0);
                    if (!empty($curr_dep['date'])) {
                        $payment_date = $curr_dep['date'];
                    }
                }

                $rec_no = $c_item['invoice_id'] . '/' . $c_item['sub_invoice_id'];
                if (!in_array($rec_no, $receipt_nos)) {
                    $receipt_nos[] = $rec_no;
                }
            }
            ...
            if (!empty($clean_mobile) && strlen($clean_mobile) == 10) {
                $pending_payment_str = (floor($overall_pending) == $overall_pending) ? number_format($overall_pending, 0, '.', '') : number_format($overall_pending, 2, '.', '');
                $amount_paid_str = (floor($total_paid_amount) == $total_paid_amount) ? number_format($total_paid_amount, 0, '.', '') : number_format($total_paid_amount, 2, '.', '');
                $fee_type_str = implode(', ', $fee_types);
                $receipt_no_str = implode(', ', $receipt_nos);
                $formatted_date = !empty($payment_date) ? date('d-m-Y', strtotime($payment_date)) : date('d-m-Y');
                $school_name = 'SVN JAGIROAD';

                $message = "Dear Parent,\nFee payment has been received successfully.\nStudent: " . $student_name . "\nFee Type: " . $fee_type_str . "\nAmount Paid: Rs" . $amount_paid_str . "\nPending Payment: " . $pending_payment_str . "\nReceipt No: " . $receipt_no_str . "\nDate: " . $formatted_date . "\nThank you.\n- " . $school_name . "\n\nPowerd by KKWEBMART\nDE";
    ...
    public function getFeeTypeCollectionSummaryBetweenDates($start_date, $end_date)
    {
        $this->db->select('fee_groups.id as fee_groups_id, fee_groups.name as fee_group_name, fee_groups.is_system as fee_group_is_system, '
            . 'feetype.id as feetype_id, feetype.type as fee_type, feetype.is_system, student_fees_deposite.amount_detail');
        ...
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('fee_groups_feetype.session_id', $this->current_session);
        $this->db->group_by('fee_groups.id, feetype.id, student_fees_deposite.id');
        $query = $this->db->get();
        $result = $query->result();
        ...
        foreach ($result as $row) {
            ...
            foreach ($amount_detail as $payment) {
                ...
                $pay_ts = strtotime($payment->date);
                if ($pay_ts < $start_ts || $pay_ts > $end_ts) {
                    continue;
                }

                if (!isset($summary[$group_key])) {
                    $fee_type_label = $row->is_system ? $this->lang->line($row->fee_type) : $row->fee_type;
                ...
                $payment_mode = isset($payment->payment_mode) ? strtolower($payment->payment_mode) : 'cash';
                $amount = (float) $payment->amount;

                if ($payment_mode === 'upi') {
                    $summary[$group_key]['upi_amount'] += $amount;
                } else {
                    $summary[$group_key]['cash_amount'] += $amount;
                }

                $summary[$group_key]['total_amount'] += $amount;
            }
        }

        uasort($summary, function ($a, $b) {
            return strcasecmp($a['fee_type'], $b['fee_type']);
        });
    ...
    public function fee_deposit_collections($data)
    {
        if (!empty($data)) {
            ...
            // Custom SMS API Trigger for Selected / Bulk Collections
            try {
                if (!empty($collected_fees) && is_array($collected_fees)) {
                    $this->send_fee_sms_custom_bulk($collected_fees, $data);
                }
            } catch (Exception $e) {
                // Silently fail to not disrupt fee deposit flow
            }
