...
class Mailsms extends Admin_Controller
{
    ...
    private function check_fee_reminder_menu()
    {
        $chk = $this->db->get_where('sidebar_sub_menus', array('url' => 'admin/mailsms/fee_reminder'))->row();
        if (!$chk) {
            $parent = $this->db->where('lang_key', 'communicate')->or_where('key', 'communicate')->get('sidebar_menus')->row();
            if (!$parent) {
                $parent = $this->db->like('menu', 'Communicate')->get('sidebar_menus')->row();
            }
            if ($parent) {
                $data = array(
                    'sidebar_menu_id'     => $parent->id,
                    'url'                 => 'admin/mailsms/fee_reminder',
                    'lang_key'            => 'fee_reminder',
                    'menu'                => 'Reminder',
                    'activate_controller' => 'mailsms',
                    'activate_methods'    => 'fee_reminder',
    ...
    public function fee_reminder()
    {
        if (!$this->rbac->hasPrivilege('email', 'can_view') && !$this->rbac->hasPrivilege('email_sms_log', 'can_view')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'Communicate');
        $this->session->set_userdata('sub_menu', 'admin/mailsms/fee_reminder');
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (!empty($students)) {
                    foreach ($students as $key => $student) {
                        $st_session_id = $student['student_session_id'];

                        // Calculate Regular Fees
                        $student_fee_list = $this->studentfeemaster_model->getStudentFees($st_session_id);
                        $tot_assigned = 0;
                        $tot_paid = 0;

                        if (!empty($student_fee_list)) {
                            foreach ($student_fee_list as $m_fee) {
                                $fees_arr = is_array($m_fee->fees) ? $m_fee->fees : (is_object($m_fee->fees) ? get_object_vars($m_fee->fees) : array());
                                if (!empty($fees_arr)) {
                                    foreach ($fees_arr as $f_item) {
                                        ...
                                        $tot_assigned += (float) (isset($f_item->amount) ? $f_item->amount : 0);
                        ...
                        $trans_list = $this->studentfeemaster_model->getTransStudentFees($st_session_id);
                        if (!empty($trans_list)) {
                            foreach ($trans_list as $t_fee) {
                                $tot_assigned += (float) (isset($t_fee->amount) ? $t_fee->amount : 0);
                        ...
                        $student['paid_fee']    = $tot_paid;
                        $student['pending_fee'] = max(0, $tot_assigned - $tot_paid);
                        $resultlist[]           = $student;
                    }
                }
            }
        ...
        $this->load->view('layout/header');
        $this->load->view('admin/mailsms/fee_reminder', $data);
        $this->load->view('layout/footer');
    }

    public function send_fee_reminder_sms()
    {
        if (!$this->rbac->hasPrivilege('email', 'can_view') && !$this->rbac->hasPrivilege('email_sms_log', 'can_view')) {
            access_denied();
        }
        ...
        if (empty($student_session_ids)) {
            $this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Please select at least one student.</div>');
            redirect('admin/mailsms/fee_reminder');
            return;
        }
        ...
        foreach ($student_session_ids as $st_session_id) {
            $student_rec = $this->studentsession_model->searchStudentsBySession($st_session_id);
            if (!$student_rec) {
                continue;
            }
            ...
            $class_name   = $student_rec['class'] ?? '';
            $section_name = $student_rec['section'] ?? '';
            ...
            $paid_fees    = $tot_paid;
            $pending_fees = max(0, $tot_assigned - $tot_paid);

            $paid_str    = (floor($paid_fees) == $paid_fees) ? number_format($paid_fees, 0, '.', '') : number_format($paid_fees, 2, '.', '');
            $pending_str = (floor($pending_fees) == $pending_fees) ? number_format($pending_fees, 0, '.', '') : number_format($pending_fees, 2, '.', '');
            ...
            if (!empty($clean_mobile) && strlen($clean_mobile) == 10) {
                $pending_val = "Rs " . $pending_str;
                $message     = "Dear Parent\nThis is a gentle reminder that the school fee of  " . $pending_val . " for " . $student_name . " is still pending. Kindly make the payment at your earliest convenience. - " . $school_name . "\n\nIf you have already paid or are exempted from institution, ignore it.\nPowered by KKWEBMART\nDE";

                $api_url = "http://bulk.powerstext.in/http-tokenkeyapi.php?authentic-key=34384b4b5745424d4152543130301773909499&senderid=DSAENT&route=1&number=" . $clean_mobile . "&message=" . urlencode($message) . "&templateid=1677100000000389485";
                ...
                if (isset($this->smsgateway)) {
                    @$this->smsgateway->sendSMS($clean_mobile, $message, '1677100000000389485');
                }
        ...
        $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Fee reminder SMS sent successfully to ' . $sent_count . ' student(s).</div>');
        redirect('admin/mailsms/fee_reminder');
    }
