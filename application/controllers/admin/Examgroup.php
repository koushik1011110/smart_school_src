...
class Examgroup extends Admin_Controller
{
    ...
    public function exportbulkformat()
    {
        ...
        if (!empty($exam_id) && !empty($class_id) && !empty($section_id) && !empty($session_id)) {
            $students = $this->examgroupstudent_model->searchExamStudents(0, $exam_id, $class_id, $section_id, $session_id);
            $subjects = $this->batchsubject_model->getExamSubjects($exam_id);

            if (!empty($students) && !empty($subjects)) {
                $existing_map = array();
                foreach ($subjects as $sub) {
                    $results = $this->examgroupstudent_model->examGroupSubjectResult($sub->id, $class_id, $section_id, $session_id);
                    if (!empty($results)) {
                        foreach ($results as $res) {
                            $std_id = $res['exam_group_class_batch_exam_students_id'];
                            $val = ($res['exam_group_exam_result_attendance'] == 'absent') ? 'A' : $res['exam_group_exam_result_get_marks'];
                            $existing_map[$std_id][$sub->id] = $val;
                        }
                    }
                }

                $header = array('adm_no', 'student_name');
                foreach ($subjects as $sub) {
                    $sub_head = !empty($sub->subject_code) ? $sub->subject_code : $sub->subject_name;
                    $header[] = str_replace(',', ' ', $sub_head);
                }
                $csv_data = '"' . implode('","', $header) . '"' . "\n";

                foreach ($students as $std) {
                    $adm_no       = !empty($std->admission_no) ? $std->admission_no : $std->roll_no;
                    $student_name = $this->customlib->getFullName($std->firstname, $std->middlename, $std->lastname, $this->sch_setting_detail->middlename, $this->sch_setting_detail->lastname);
                    $student_name = str_replace(',', ' ', $student_name);

                    $row    = array($adm_no, $student_name);
                    $std_id = $std->exam_group_class_batch_exam_student_id;

                    foreach ($subjects as $sub) {
                        $val   = isset($existing_map[$std_id][$sub->id]) ? $existing_map[$std_id][$sub->id] : '';
                        $row[] = $val;
                    }
                    $csv_data .= '"' . implode('","', $row) . '"' . "\n";
                }

                $name = 'exam_marks_matrix_sample.csv';
                force_download($name, $csv_data);
                return;
            }
        }

        $csv_data = "adm_no,student_name,MATH101,SCI101,ENG101\n";
        $csv_data .= '1001,"John Doe",75,80,85' . "\n";
        $csv_data .= '1002,"Jane Smith",65,A,90' . "\n";
        $name = 'exam_marks_matrix_sample.csv';
        force_download($name, $csv_data);
    }
    ...
    public function uploadbulkfile()
    {
        ...
        if (!isset($_FILES["file"]) || empty($_FILES['file']['name'])) {
            $this->form_validation->set_rules('file', $this->lang->line('file'), 'required');
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = array('status' => '0', 'error' => implode(' ', array_filter($data)));
        ...
        if (!in_array($mtype, $allowed_mime_type) || !in_array($ext, $allowed_extension) || !in_array($file_type, $allowed_mime_type)) {
            $array = array('status' => '0', 'error' => $this->lang->line('extension_not_allowed'));
            echo json_encode($array);
            return;
        }
        ...
        $students = $this->examgroupstudent_model->searchExamStudents(0, $exam_id, $class_id, $section_id, $session_id);
        if (empty($students)) {
            $array = array('status' => '0', 'error' => $this->lang->line('no_record_found'));
            echo json_encode($array);
            return;
        }
        ...
        $student_map = array();
        foreach ($students as $std) {
            if (!empty($std->admission_no)) {
                $student_map[trim($std->admission_no)] = $std->exam_group_class_batch_exam_student_id;
            }
            if (!empty($std->roll_no)) {
                $student_map[trim($std->roll_no)] = $std->exam_group_class_batch_exam_student_id;
            }
        }

        $subjects = $this->batchsubject_model->getExamSubjects($exam_id);
        if (empty($subjects)) {
            $array = array('status' => '0', 'error' => 'No subjects found for this exam.');
            echo json_encode($array);
            return;
        }
        ...
        foreach ($subjects as $sub) {
            if (!empty($sub->subject_code)) {
                $subject_map[strtolower(trim($sub->subject_code))] = $sub->id;
            }
            if (!empty($sub->subject_name)) {
                $subject_map[strtolower(trim($sub->subject_name))] = $sub->id;
            }
        }
        ...
        if (!$header || count($header) < 3) {
            fclose($file);
            $array = array('status' => '0', 'error' => 'Invalid CSV format.');
            echo json_encode($array);
            return;
        }

        $col_subject_map = array();
        for ($i = 2; $i < count($header); $i++) {
            $sub_key = strtolower(trim($header[$i]));
            if (isset($subject_map[$sub_key])) {
                $col_subject_map[$i] = $subject_map[$sub_key];
            }
        }

        if (empty($col_subject_map)) {
            fclose($file);
            $array = array('status' => '0', 'error' => 'No matching subject headers found in CSV.');
            echo json_encode($array);
            return;
        }

        while (($column = fgetcsv($file, 10000, ",")) !== false) {
            $adm_no = isset($column[0]) ? trim($column[0]) : '';
            if ($adm_no === '' || !isset($student_map[$adm_no])) {
                continue;
            }

            $student_id = $student_map[$adm_no];

            foreach ($col_subject_map as $col_index => $subject_id) {
                if (isset($column[$col_index])) {
                    $cell_val = trim($column[$col_index]);
                    if ($cell_val === '') {
                        continue;
                    }

                    if (strtolower($cell_val) === 'a' || strtolower($cell_val) === 'absent') {
                        $attendance = 'absent';
                        $marks      = '0';
                    } else {
                        $attendance = 'present';
                        $marks      = number_format((float)$cell_val, 2, '.', '');
                    }
        ...
        if (!empty($insert_array)) {
            ...
            $array = array('status' => '1', 'error' => '', 'message' => count($insert_array) . ' marks records imported successfully.');
            echo json_encode($array);
        } else {
            $array = array('status' => '0', 'error' => 'No marks records were found to import in the uploaded file.');
            echo json_encode($array);
        }
    }
