...
class Feemaster extends Admin_Controller
{
    ...
    public function copyToSession()
    {
        ...
        $target_session = $this->input->post('target_session');

        if ($target_session && $target_session != $current_session) {
            $result = $this->feesessiongroup_model->copyToSession($current_session, $target_session);
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Please select a different session</div>');
        }
