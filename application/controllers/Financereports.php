...
class Financereports extends Admin_Controller
{
    ...
    public function feetype_collection_summary()
    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/feetype_collection_summary');

        $data = array();
        $data['title'] = 'Fee Type Collection Summary';
        $data['search_type'] = $this->input->post('period') ? $this->input->post('period') : 'today';

        $end_date = date('Y-m-d');
        if ($data['search_type'] === 'today') {
            $start_date = $end_date;
        } elseif ($data['search_type'] === 'last_week') {
            $start_date = date('Y-m-d', strtotime('-6 days'));
        } elseif ($data['search_type'] === 'last_month') {
            $start_date = date('Y-m-d', strtotime('-1 month'));
        } elseif ($data['search_type'] === 'last_year') {
            $start_date = date('Y-m-d', strtotime('-1 year'));
        } else {
            $data['search_type'] = 'today';
            $start_date = $end_date;
        }

        $data['start_date']  = $start_date;
        $data['end_date']    = $end_date;
        $data['report_data'] = $this->studentfeemaster_model->getFeeTypeCollectionSummaryBetweenDates($start_date, $end_date);

        $this->load->view('layout/header', $data);
        $this->load->view('financereports/feetype_collection_summary', $data);
        $this->load->view('layout/footer', $data);
    }
