...
class Studentfee extends Admin_Controller
{
    ...
    public function feesearch()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Sort student list by roll number in ascending order
            uasort($students, function ($a, $b) {
                $a_val = preg_replace('/[^0-9]/', '', $a['roll_no']);
                $b_val = preg_replace('/[^0-9]/', '', $b['roll_no']);
                $a_num = $a_val !== '' ? (int)$a_val : 0;
                $b_num = $b_val !== '' ? (int)$b_val : 0;
                if ($a_num !== $b_num) {
                    return $a_num - $b_num;
                }
                return strcmp($a['roll_no'], $b['roll_no']);
            });
    ...
    public function schoolerPrintByGroupArray()
    {
        $data['sch_setting'] = $this->sch_setting_detail;
        $record = $this->input->post('data');
        $record_array = json_decode($record);
        ...
        $data['feearray'] = $fees_array;
        $this->load->view('print/schoolerPrintFeesByGroupArray', $data);
    }
