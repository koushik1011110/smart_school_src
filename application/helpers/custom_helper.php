...
function amountFormat($amount, $decimals = null, $dec_point = null, $thousands_sep = null)
{
    $CI              = &get_instance();
    ...
    // Determine decimal points: 0 if integer, else use passed decimals or default to 2
    $decimal_points = ($amount == (int)$amount) ? 0 : ($decimals ?? 2);
    if ($currency_format == "#,###.###" && $decimal_points != 0) {
        $decimal_points = 3;
    }

    if ($currency_format == "#,###.##") {
        $return_amt = number_format($amount, $decimal_points, $dec_point ?? '.', $thousands_sep ?? ',');
